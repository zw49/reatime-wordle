...
import { createSession, useRandomPuzzle } from "../components/actions";
...
function Home() {
  ...
  const [multiplayerPuzzle, setMultiplayerPuzzle] = useState("");
  const navigate = useNavigate();

  const handleMultiplayerButtonClick = async () => {
    setMultiplayerDialogOpen(true)
    const { data: puzzle, error } = await supabase.from("random_puzzle").select("*")
    console.log(puzzle)
    setMultiplayerPuzzle(puzzle[0].id)
    const session = await createSession()
    setMultiplayerLink(`${session}&puzzleId=${puzzle[0].id}`)
  }
  return (
    <div className="h-full w-full flex gap-2 items-center justify-center">
      ...
      <Dialog open={multiplayerDialogOpen} onClose={() => setMultiplayerDialogOpen(false)} className="relative z-50">
